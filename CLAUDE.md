...
(currently `0.1.0-alpha.12`). It transitively pulls in `@peerkit/api`,
...
- `@peerkit/relay` exports `run(RelayConfig): Promise<void>` (high-level
  ...
  `shutdownRelayMetrics`, `setAgentCountProvider`, and types `RelayConfig`
  and `OtelConfig`. `run()` owns logger + metrics + `MemoryAgentStore` +
  relay startup + SIGINT/SIGTERM handlers and calls `process.exit` on
  shutdown.
- `RelayConfig` requires `id`, `logLevel`, `listenAddrs`,
  `networkAccessBytes`, `networkAccessHandler`. Optional: `publicHost`,
  `otel`. `OtelConfig` requires `otlpEndpoint` and `serviceVersion`;
  optional `exportIntervalMs`, `headers`.
